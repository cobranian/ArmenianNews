...
| Section | Source | How |
|---|---|---|
| **Actualités** | [Armenpress](https://armenpress.am/fr) | The national news agency's latest **16 headlines per language** (fr / en / hy), as a single **Fil / Wire / Հոսք** shelf. The only source here that is **trilingual** — fr/en/hy map 1:1 to the UI language — but not the default tab: Courrier d'Erevan prerenders far more French copy (80 articles vs. 16). Armenpress is an Inertia.js app, so its feed arrives as embedded JSON — **no CSS selectors**. Read from the homepage, not the rubric pages (those embed an empty feed and load client-side), with three spaced requests: the site rate-limits hard. |
